...
for tweet in dic_idword.values():
    ...
    # print(tweet, rating, list(dic_idword.keys())[list(dic_idword.values()).index(tweet)])
# print(rating_list)
# dict_idrating = dict(zip(id_list, rating_list))
with open('ID RATING.txt', 'w') as txt2:
    for a, b in dict_idrating.items():
        txt2.write('%s : %s\n' % (a, b))
